...
KN86_TEST(field_caret_past_text_is_solid_block)
{
    ...
    KN86_ASSERT(ui_cell_solid(2, 0, UI_EMBER));        /* caret over space */
    kec_close(S);
}
...
KN86_TEST(field_no_caret_is_readonly_echo)
{
    ...
    KN86_ASSERT(!ui_cell_solid(2, 0, UI_EMBER));
...
KN86_TEST(form_focused_field_has_caret)
{
    ...
    KN86_ASSERT(ui_cell_solid(9, 0, UI_EMBER));        /* caret past "abe" */
    /* unfocused field 1 value "10" at col 6 row 1, NO caret. */
    KN86_ASSERT(ui_glyph_normal(6, 1, '1'));
    KN86_ASSERT(!ui_cell_solid(8, 1, UI_EMBER));
    kec_close(S);
}
...
KN86_TEST(slider_cursor_at_value)
{
    ...
    KN86_ASSERT(ui_cell_solid(4, 3, UI_EMBER));        /* cursor */
...
KN86_TEST(slider_clamps_to_ends)
{
    ...
    KN86_ASSERT(ui_cell_solid(4, 0, UI_EMBER));
    kec_close(S);
}
...
KN86_TEST(dial_fills_and_reads_out)
{
    ...
    KN86_ASSERT_EQ(render_get_pixel(12, 38), UI_EMBER);   /* in the fill */
...
KN86_TEST(cycler_marks_current_option)
{
    kec_State *S = ui_open();
    fe_Context *ctx = kec_fe(S);
    /* MODE: EMBER > WHITE > GREEN, current "WHITE". label width 4 ->
     * options start at col 6. "EMBER"(5) + ">"(1) -> WHITE starts col 12. */
    ui_eval(ctx,
        "(ui/cycler 0 0 \"MODE\" \"WHITE\" '(\"EMBER\" \"WHITE\" \"GREEN\"))");
    /* "EMBER" is the un-selected option: normal 'E' at col 6. */
    KN86_ASSERT(ui_glyph_normal(6, 0, 'E'));
...
KN86_TEST(toast_is_centered_inverted_band)
{
    ...
    KN86_ASSERT(ui_cell_solid(60, 72, UI_EMBER));      /* left pad lit */
