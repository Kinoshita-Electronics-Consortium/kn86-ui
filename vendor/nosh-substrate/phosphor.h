 ...
 *
 *   AMBER  #E6A020 -> RGB565 0xE504   canonical default (P3-ish).
 *   WHITE  #F0F0F0 -> RGB565 0xF79E   alternate (P4-style).
 *   GREEN  #33F033 -> RGB565 0x3FE6   alternate (P1-style).
 *
 * Background is fixed black 0x0000. AMBER is locked; WHITE/GREEN are
 ...
 * value set `"amber" | "white" | "green"` — the ADR-0036 amendment of
...
typedef enum {
    PHOSPHOR_AMBER = 0,   /* #E6A020 — canonical default */
...
#define PHOSPHOR_RGB565_AMBER  ((uint16_t)0xE504)
...
/* The currently active scheme. Defaults to PHOSPHOR_AMBER at process
 * start (the canonical default) until phosphor_set() or a config load
 * changes it. */
PhosphorScheme phosphor_active(void);

/* Switch the active scheme. Out-of-range values clamp to AMBER. Does
 * NOT persist — call phosphor_config_save() to write the choice. */
void phosphor_set(PhosphorScheme scheme);

/* The RGB565 foreground for a scheme. Out-of-range returns the AMBER
...
/* Map a lowercase scheme name ("amber"|"white"|"green") to the enum.
 * Unknown / wrong-case / empty / NULL falls back to PHOSPHOR_AMBER. */
PhosphorScheme phosphor_from_name(const char *name);

/* Stable lowercase name ("amber"/"white"/"green"), for the config file.
 * Always non-NULL; out-of-range returns "amber". */
const char *phosphor_to_name(PhosphorScheme scheme);

/* The Fe keyword symbol text (":amber"/":white"/":green") that the
 ...
 * active scheme. Returns the scheme loaded (PHOSPHOR_AMBER on a missing
 * file / table / key, or an invalid value). A NULL path is treated as a
 * missing file (returns AMBER, leaves the active scheme unchanged). */
